...
List<TextEditingController> purchaseCartController2=[];
double purchaseTotal=0;
List<double> purchaseCartTotalList=[];
List<String> purchaseCartUomList=[];
...
class PurchaseScreen extends StatefulWidget {
    static const String id='purchase';
    @override
...
class _PurchaseScreenState extends State<PurchaseScreen> {
    ...
    void addToCart(int index){
        ...
        if(purchaseCartListText.isEmpty){
            ...
            purchaseCartController2.add(TextEditingController());
            tempText='${productNameF[index]}:$uom:$priceList: 1';
            setState(() {
                purchaseCartTotalList.add(double.parse(priceList.trimLeft()));
                purchaseCartController[0].text=priceList;
                purchaseCartController2[0].text=priceList;
                purchaseCartUomList.add(uom);
                purchaseCartListText.add(tempText.trim());
            });
        }
        else {
            for(int i=0;i<purchaseCartListText.length;i++) {
                List tempCart=purchaseCartListText[i].split(':');
                if (tempCart[0]==productNameF[index]) {
                    ...
                    if (tempList[1].toString().trim() ==
                        ...
                        setState(() {
                            purchaseCartTotalList[i] = double.parse(tempRate.toString());
    ...
    void addFromSearch(String name,String quantityValue){
        ...
        if(purchaseCartListText.isEmpty){
            ...
            setState(() {
                purchaseCartTotalList.add(tempRate);
                purchaseCartController[0].text=tempRate.toString();
                purchaseCartController2[0].text=tempRate.toString();
                purchaseCartUomList.add(getBaseUom(name));
                purchaseCartListText.add(tempText.trim());
            });
        }
        else {
            for (int i = 0; i < purchaseCartListText.length; i++) {
                ...
                if (tempCart[0]==name) {
                    ...
                    if (tempList[1].toString().trim() ==
                        ...
                        tempText = tempText.replaceAll(new RegExp(r"\s+"), " ");
                        setState(() {
                            purchaseCartTotalList[i] = tempRate;
                            purchaseCartController[i].text = tempRate.toString();
                            purchaseCartListText[i] = tempText.trim();
                        });
                        return;
            ...
            if (tempCart[0]==text) {
                ...
                if (tempList[1].toString().trim() ==
                    ...
                    if (tempQuantity == 0 || tempQuantity.isNegative) {
                        setState(() {
                            purchaseCartTotalList.removeAt(i);
                            purchaseCartController.removeAt(i);
                            purchaseCartController2.removeAt(i);
                            purchaseCartListText.removeAt(i);
                            purchaseCartUomList.removeAt(i);
                        });
                        return;
                    }
                    else {
                       double tempRate =
                           ...
                           double.parse(tempList[2])/double.parse(tempList[3]);
                        tempRate = tempRate * tempQuantity;
                        tempList[2] = '$tempRate';
                        tempList[3] = tempQuantity.toString();
                        tempText = tempList.toString();
        ...
        for(int i=0;i<purchaseCartListText.length;i++) {
            ...
            if (tempCart[0]==text) {
                ...
                if (tempList[1].toString().trim() ==
                    ...
                    double tempRate = double.parse(tempList[2])/double.parse(tempList[3]);
                    tempQuantity = tempQuantity + 1;
                    ...
                    for (int j = 0; j < tempBarcodeSplit.length; j++) {
                        if (tempBarcodeSplit[j] == barcodeVal) {
                            ...
                            setState(() {
                                ...
                                purchaseCartTotalList.add(double.parse(tempPrice));
                                purchaseCartController.add(TextEditingController(
                                    text: tempPrice
                                ));
                                purchaseCartUomList.add(tempUomSplit[j]);
                                purchaseCartListText.add(tempText.trim());
                            });
                            print(tempText);
                            return;
                ...
                if (productFirstSplit[i].toString().contains(tempBarcode)){
                    ...
                    if (barcodeType == 'selectedMode.Weighted') {
                        ...
                        setState(() {
                            ...
                            purchaseCartTotalList.add(tempPrice);
                            purchaseCartUomList.add(uom[0]);
                            purchaseCartListText.add(tempText.trim());
                        });
                        print(tempText);
                        return;
    ...
    String getBaseUom(String productName){
        for(int i=0;i<productFirstSplit.length;i++){
            List temp111=productFirstSplit[i].toString().split(':');
            if(temp111[0].toString().trim()==productName){
                print('inside get base uom productName $productName');
                print('inside get base uom temp111[0] ${temp111[0]}');
                List temp=productFirstSplit[i].split(':');
                List tempUom=temp[4].split('``');
                List tempUomSplit=tempUom[0].toString().split('*');
                print('inside get base uom ${tempUomSplit[0]}');
    ...
    Future checkOut() async {
        ...
        if(invEdit.value){
            ...
            if(invEditVendorName.value.length>0){
                DocumentSnapshot documentSnapshot=await firebaseFirestore.collection('vendor_report').doc(invEditVendorName.value).get();
        ...
        print('cartListText $purchaseCartListText');
        double tot = getTotal(purchaseCartTotalList);
        String tempOrder;
        int invNo;
        invNo=await getLastInv('purchase');
        String tempEditInv='';
        if(invEdit.value){
            tempEditInv=invEditNumber.value;
        }
        else{
            tempEditInv='$purchasePrefix$invNo';
        }
        List yourItemsList1=[];
        balance=selectedPurchasePayment=='Credit'?purchaseTotal:0;
        print('selectedPurchasePayment $selectedPurchasePayment');
        selectedVendor=selectedVendor.length>0?selectedVendor:'Standard';
        String body='$tempEditInv~${dateNow()}~$selectedVendor~$selectedPurchasePayment~$purchaseTotal~Purchase~$currentUser~$balance';
        yourItemsList1.add({
            "invNo":"$tempEditInv",
        ...
        //
        // }
       // await insertData(body,'purchase_data');
        String vat5Body='';
        double taxable5=0;
        double tax5=0;
        double total5=0;
        ...
        if(invEdit.value){
            ...
            create(body, 'purchase', yourItemsList);
            updateInv('purchase', invNo+1);
        }
        ...
        String vatBody='$tempEditInv~${dateNow()}~$selectedVendor~${getVat(selectedVendor)}~$purchaseTotal~${taxable5.toStringAsFixed(decimals)}~${tax5.toStringAsFixed(decimals)}~$total5~$total0~purchase';
        create(vatBody, 'vat_report', []);
        if(selectedPurchasePayment=='Credit'){
            updateReport(selectedVendor, purchaseTotal.toStringAsFixed(3), 'vendor_data','',getVendorBalance(selectedVendor),'purchase');
        }
        for(int k=0;k<purchaseCartListText.length;k++){
            List temp=purchaseCartListText[k].split(':');
            String itemName=temp[0];
            ...
            String itemUom=temp[1];
            String itemConversion=getConversion(itemName, itemUom);
            itemConversion=='0'?itemConversion='1':itemConversion=itemConversion;
            double tempQty=double.parse(itemQty)*double.parse(itemConversion);
            itemQty=tempQty.toString();
            String stockDetails=await readStock(itemName);
            List itemStockDetailsSplit=stockDetails.split('~');
            ...
            double newCostPrice;
            if(costPrice=='0'){
                newCostPrice=double.parse(itemPrice)/double.parse(itemQty);
            }
            else{
                double nominatorA=double.parse(stockQuantity).abs()*double.parse(costPrice);
                print('nominatorA $nominatorA');
                double nominatorB=double.parse(itemPrice);
                print('nominatorB $nominatorB');
                double denominator=double.parse(stockQuantity).abs()+double.parse(itemQty);
                print('denominator $denominator');
                newCostPrice=(nominatorA+nominatorB)/denominator;
            }
            double newStockValue=(double.parse(stockQuantity)+double.parse(itemQty))*newCostPrice;
            print('newCostPrice $newCostPrice');
            String stockBody='$itemName~${double.parse(stockQuantity)+double.parse(itemQty)}~${newCostPrice.toStringAsFixed(3)}~${newStockValue.toStringAsFixed(3)}';
            print('stockBody $stockBody');
             updateStock(stockBody);
                                                                    ...
                                                                    children: [
                                                                        Expanded(
                                                                            child: Container(
                                                                                ...
                                                                                //     suggestions: allPurchasableProducts,
                                                                                //     clearOnSubmit: false,
                                                                                //     textSubmitted:  (text) {
                                                                                //         _selectedItem=text;
                                                                                //         if(allPurchasableProducts.contains(_selectedItem)) {
                                                                                //             setState(() {
                                                                    ...
                                                                    children: [
                                                                        Row(
                                                                            ...
                                                                            children: [
                                                                                ...
                                                                                Text(getTotal(purchaseCartTotalList).toStringAsFixed(decimals), style: TextStyle(
                                                                                    fontWeight: FontWeight.bold,
                                                                                    fontSize: MediaQuery.of(context).textScaleFactor*20,
                                                                                ),),
                                                                            ],
                                                                                    ...
                                                                                    ElevatedButton(
                                                                                        ...
                                                                                        onPressed: (){
                                                                                            setState(() {
                                                                                                ...
                                                                                                // purchaseOrderNo=purchaseOrderNoList[purchaseOrderNoList.length-1]+1;
                                                                                                purchaseCartController=[];
                                                                                                purchaseCartController2=[];
                                                                                                purchaseCartUomList=[];
                                                                                                purchaseCartTotalList=[];
                                                                                                print(purchaseOrderNoList);
                                                                                            });
                                                                                            invEdit.value=false;
                                                                                        },
    ...
    DataTable dataTable(){
        ...
        return DataTable(
            ...
            ], rows: List.generate(purchaseCartListText.length, (index) { 
                double tprice = double.parse(purchaseCartController[index].text)*double.parse(getItem(index, 3));
                return DataRow(
                    cells: [
            ...
            DataCell( DropdownButton(
                value: purchaseCartUomList[index].toString().trim(),// Not necessary for Option 1
                ...
                onChanged: (newValue) {
                    ...
                    print(tempVal);
                    setState(() {
                        purchaseCartTotalList[index]=tempPrice1;
                        purchaseCartController[index].text=tempPrice1.toString();
                        purchaseCartUomList[index] = newValue;
                    ...
                    GestureDetector(
                        ...
                        DataCell(
                            TextFormField(
                                ...
                                controller: purchaseCartController2[index],
                                onChanged: (val){
                                    List showCartItems=purchaseCartListText[index].split(':');
                                    double ab = double.parse(val)*double.parse(showCartItems[3]);
                                    ...
                                    print(tempVal);
                                    setState(() {
                                        purchaseCartTotalList[index]=ab;
                                        purchaseCartListText[index]=tempVal;
                                        purchaseCartController[index].text = ab.toString();
                                    });
                                },),
                            showEditIcon: true
                        ),
            DataCell(
                TextFormField(
                    ...
                    onChanged: (val){
                        ...
                        print(tempVal);
                        setState(() {
                            purchaseCartTotalList[index]=double.parse(val);
                            purchaseCartListText[index]=tempVal;
                        });
                    },),
                showEditIcon: true
            ...
            DataCell(IconButton(
                onPressed: (){
                    setState(() {
                        purchaseCartTotalList.removeAt(index);
                        purchaseCartController.removeAt(index);
                        purchaseCartController2.removeAt(index);
                        purchaseCartListText.removeAt(index);
                        purchaseCartUomList.removeAt(index);
                    });
                },
...
//   print('items:$items');
//   print('inside all $purchaseOrderNoList');
//   if(purchaseOrderNoList.contains(orderNumber)){
//     for(int i=0;i<savedOrders.length;i++){
//       if(savedOrders[i].contains(orderNumber.toString())){
