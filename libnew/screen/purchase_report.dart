                            ...
                            return DataRow(cells: [
                              ...
                              DataCell(IconButton(onPressed: () async {
                                ...
                                if(transactionType=='purchase'){
                                  ...
                                  pr.invEdit.value=true;
                                  pr.invEditNumber.value=document['orderNo'];
                                  List items=document['cartList'];
                                  pr.invEditCartList.value=RxList<String>([]);
                                  pr.purchaseCartController=[];
                                  pr.purchaseReturnUomList=[];
                                  pr.purchaseReturnTotalList=[];
                                  pr.purchaseReturnTotal=0;
                                  pr.purchaseCartListText=[];
                                  for(int i=0;i<items.length;i++)
                                  {
                                    setState((){
                                      pr.purchaseReturnTotal+=double.parse(document['cartList'][i]['price']);
                                      pr.purchaseReturnTotalList.add(double.parse(document['cartList'][i]['price']));
                                      pr.purchaseReturnUomList.add(document['cartList'][i]['uom']);
                                      pr.purchaseCartController.add(TextEditingController(text: document['cartList'][i]['price']));
                                      pr.purchaseCartListText.add('${document['cartList'][i]['name']}:${document['cartList'][i]['uom']}:${document['cartList'][i]['price']}:${document['cartList'][i]['qty']}');
                                    });
                                    pr.invEditCartList.value.add('${document['cartList'][i]['name']}:${document['cartList'][i]['uom']}:${document['cartList'][i]['price']}:${document['cartList'][i]['qty']}');
                                  }
                                  pr.appbarVendorController.text=document['vendor']=='Standard'?'':document['vendor'];
                                  pr.invEditVendorName.value=selectedVendor=pr.appbarVendorController.text;
                                  pr.invEditPaymentMethod.value=document['payment'];
                                  pr.invDate.value=document['date'];
                                  pr.invEditTotal.value=document['total'];
                                  await displayAllProducts('Purchasable');
                                  Navigator.pushReplacement(
                                    context, MaterialPageRoute(builder: (context) => pr.PurchaseReturn()),
                                  );
                                }
                                  ...
                                  pr.invEditPaymentMethod.value=document['payment'];
                                  sr.invDate.value=document['date'];
                                  pr.invEditTotal.value=document['total'];
