...
Future uploadCustomer(List name,List address,List flat,List bld,List block,List road,List area,List landmark,List note,List mobile,List gst,List balance,List priceList,List route)async{
  for(int i=0;i<name.length;i++){
    firebaseFirestore
      ...
      'landmark': landmark[i],
      'note': note[i],
      'route':route[i].toString().trim().substring(0,route[i].toString().trim().length-1)
    }).then((_) {
      print('success');
    });
  }
...
Future create(String body, String collectionName) async {
  ...
  else if(collectionName=='route_data'){
    firebaseFirestore.collection("$collectionName").doc('$body').set(
        {
          'routeName': body,
        }).then((_){
...
Future read(String collectionName)async {
  ...
  if(collectionName=='route_data'){
   currentroutes=[];
    await firebaseFirestore.collection("$collectionName").get().then((querySnapshot) {
      querySnapshot.docs.forEach((result) {
        currentroutes.add(result.get('routeName'));
      });
    });
    selectedcurrentroute = currentroutes[0];
    print(currentroutes);
  }
  else if(collectionName=='branch_data'){
    branchList=[];
